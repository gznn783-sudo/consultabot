...
MAX_CODILO_REQUESTS = int(os.getenv("MAX_CODILO_REQUESTS", "300"))
...
UF_TRIBUNAIS = {
    # IMPORTANTE:
    # A lista abaixo NÃO filtra mais os tribunais.
    # Ela só coloca os tribunais mais prováveis primeiro.
    # Para OAB RS/SC, também priorizo os dois estaduais + TRF4,
    # porque muitos advogados aparecem em processos cruzados entre RS/SC/PR.
    "RS": ["tjrs", "tjsc", "tjpr", "trf4", "trt4", "trt12", "trt9"],
    "SC": ["tjsc", "tjrs", "tjpr", "trf4", "trt12", "trt4", "trt9"],
    "PR": ["tjpr", "tjsc", "tjrs", "trf4", "trt9", "trt12", "trt4"],
    "GO": ["tjgo", "trf1", "trt18"],
    "TO": ["tjto", "trf1"],
    "DF": ["tjdft", "trf1", "trt10"],
    "MG": ["tjmg", "trf1", "trf6", "trt3"],
...
def prioridade_search(search, prioridade):
    """Retorna uma pontuação para ordenar tribunais da UF primeiro.

    Importante: algumas rotas da Codilo podem não vir exatamente como "tjrs" ou "tjsc".
    Podem vir como variações contendo o nome do tribunal. Por isso, além do match exato,
    também aceitamos contains.
    """
    search = str(search or "").lower()

    if search in TRIBUNAIS_IGNORAR:
        return 9999

    for idx, alvo in enumerate(prioridade):
        alvo = str(alvo).lower()

        if search == alvo:
            return idx

        if alvo in search or search in alvo:
            return idx + 20
...
def ordenar_por_uf(consultas, uf=None):
    """Prioriza a UF detectada, mas NÃO descarta os demais tribunais."""
    consultas = [
        c for c in consultas
        if str(c.get("search", "")).lower() not in TRIBUNAIS_IGNORAR
    ]
    ...
    return sorted(
        consultas,
        key=lambda item: prioridade_search(item.get("search"), prioridade),
    )
...
def find_queries(param_keys, uf=None):
    ...
    todos_searches = sorted(list(set([str(c.get("search", "")).lower() for c in unicas if c.get("search")])))
    print("\n=========== TODOS SEARCHES DISPONÍVEIS NO /available ===========")
    print(", ".join(todos_searches))
    print("TEM TJRS?", any("tjrs" in t for t in todos_searches))
    print("TEM TJSC?", any("tjsc" in t for t in todos_searches))
    print("=========== FIM TODOS SEARCHES ===========\n")
...
def get_request_result(request_id, max_tentativas=12, tempo_espera=10):
    ...
    for tentativa in range(max_tentativas):
        try:
            ...
            if response.status_code == 500 and str(resultado.get("error", "")).lower().find("result cannot be resolved") >= 0:
...
def buscar_cnjs(valor, tipo):
    ...
    for item in consultas:
        try:
            search_atual = str(item.get("search", "")).lower()

            if search_atual in TRIBUNAIS_IGNORAR:
                print(f"Pulando {search_atual}: endpoint ignorado.")
                continue
