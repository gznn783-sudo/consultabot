...
CONSULTABOT_BUILD = "FALLBACK_LIMPO_SEM_HISTORICO_CRUZADO_2026_05_16"
print("========== CONSULTABOT BUILD:", CONSULTABOT_BUILD, "==========")
...
def cnj_valido(cnj):
    ...
    return bool(re.match(CNJ_REGEX, cnj))


def request_compativel(item, termo_busca=None, chave_busca=None, inicio_busca_ts=None):
    try:
        param = item.get("param", {}) or {}

        valor = str(param.get("value", "")).strip().lower()
        chave = str(param.get("key", "")).strip().lower()

        if termo_busca:
            if valor != str(termo_busca).strip().lower():
                return False

        if chave_busca:
            chaves_validas = [x.strip().lower() for x in chave_busca]
...
def parse_codilo_created_ts(value):
    try:
        if not value:
            return 0.0
        s = str(value).strip()
...
def request_e_da_busca_atual(req, item, valor=None, inicio_busca_ts=None):
    if not isinstance(req, dict):
        return False

    if str(req.get("source", "")).lower().strip() != str(item.get("source", "")).lower().strip():
        return False
    if str(req.get("platform", "")).lower().strip() != str(item.get("platform", "")).lower().strip():
        return False
    if str(req.get("search", "")).lower().strip() != str(item.get("search", "")).lower().strip():
        return False
    if str(req.get("query", "")).lower().strip() != str(item.get("query", "")).lower().strip():
        return False

    param = req.get("param", {}) or {}
    if str(param.get("key", "")).lower().strip() != str(item.get("param_key", "")).lower().strip():
        return False
    ...
    if inicio_busca_ts:
        created_ts = parse_codilo_created_ts(req.get("createdAt"))
        # margem pequena porque o relógio do Render/Codilo pode variar alguns segundos
        if created_ts and created_ts < float(inicio_busca_ts) - 15:
            return False
...
def ordenar_por_uf(consultas, uf=None, nacional=False):
    ...
    if nacional:
        print("\n=========== ROTAS MODO NACIONAL ===========")
        print("BUILD:", globals().get("CONSULTABOT_BUILD", "BUILD_FIX_SEM_NOME"))
        print("UF base:", uf)
...
# Guarda de execução atual para impedir CREATE fora do escopo regional.
CURRENT_SEARCH_CONTEXT = {
    "active": False,
    "nacional": False,
    "uf": None,
    "allowed_searches": set(),
}

def set_current_search_context(uf=None, nacional=False):
    allowed = set()
    if uf and not nacional:
        allowed = set(searches_regionais_por_uf(uf))

    CURRENT_SEARCH_CONTEXT["active"] = True
    CURRENT_SEARCH_CONTEXT["nacional"] = bool(nacional)
    CURRENT_SEARCH_CONTEXT["uf"] = (uf or "").upper().strip()
    CURRENT_SEARCH_CONTEXT["allowed_searches"] = allowed

    print("\\n=========== CONTEXTO DE BUSCA ATIVO ===========")
    print("BUILD:", globals().get("CONSULTABOT_BUILD", "BUILD_FIX_SEM_NOME"))
    print("UF:", CURRENT_SEARCH_CONTEXT["uf"])
    print("Nacional:", CURRENT_SEARCH_CONTEXT["nacional"])
    print("Allowed searches:", ", ".join(sorted(allowed)) if allowed else "TODAS")
    print("=========== FIM CONTEXTO DE BUSCA ATIVO ===========\\n")

def clear_current_search_context():
    CURRENT_SEARCH_CONTEXT["active"] = False
    CURRENT_SEARCH_CONTEXT["nacional"] = False
    CURRENT_SEARCH_CONTEXT["uf"] = None
    CURRENT_SEARCH_CONTEXT["allowed_searches"] = set()

def create_allowed_by_context(item):
    if not CURRENT_SEARCH_CONTEXT.get("active"):
        return True

    if CURRENT_SEARCH_CONTEXT.get("nacional"):
        return True

    allowed = CURRENT_SEARCH_CONTEXT.get("allowed_searches") or set()
    if not allowed:
        return True

    search = str(item.get("search", "")).lower().strip()
    return search in allowed
...
def create_request(item, value):
    if not create_allowed_by_context(item):
        print("\n=========== BLOQUEADO CREATE FORA DO ESCOPO REGIONAL ===========")
        print("BUILD:", globals().get("CONSULTABOT_BUILD", "BUILD_FIX_SEM_NOME"))
        print("UF contexto:", CURRENT_SEARCH_CONTEXT.get("uf"))
        print("Search bloqueado:", item.get("search"))
        print("Query:", item.get("query"))
        print("Platform:", item.get("platform"))
        print("Permitidas:", ", ".join(sorted(CURRENT_SEARCH_CONTEXT.get("allowed_searches") or [])))
        print("=========== FIM BLOQUEIO CREATE ===========\n")
        raise RuntimeError(f"CREATE bloqueado fora do escopo regional: {item.get('search')}/{item.get('query')}")
...
def listar_requests_fallback(item, valor=None, inicio_busca_ts=None):
    """
    Fallback profundo no histórico /v1/request.
    Agora ele NÃO usa nem exibe histórico antigo de outra pessoa/busca.
    Filtra por: source, platform, search, query, param.key, param.value e createdAt da busca atual.
    """
    todos = []
    ...
    try:
        for page in range(1, CODILO_FALLBACK_PAGES + 1):
            ...
            filtrados_pagina = [
                req for req in result
                if request_e_da_busca_atual(
                    req,
                    item,
                    valor=valor,
                    inicio_busca_ts=inicio_busca_ts,
                )
            ]

            # Log limpo: mostra só o que pertence à busca atual.
            # Isso evita aparecer Pauline/consultas antigas nos logs da busca atual.
            log_json(
                "LIST REQUEST FALLBACK FILTRADO PAGE",
                {
                    "page": page,
                    "params": params,
                    "busca": {
                    ...
                    "total_api_sem_filtro": data.get("data", {}).get("total"),
                    "recebidos_na_pagina": len(result),
                    "mantidos_apos_filtro": len(filtrados_pagina),
                    "result": filtrados_pagina,
                },
                6000,
            )

            todos.extend(filtrados_pagina)

            if not result:
                break

            total_api = data.get("data", {}).get("total")
            try:
                if total_api is not None and page * int(params["limit"]) >= int(total_api):
                    break
            except Exception:
                pass
...
def processar_rota_busca(item, valor):
    ...
    Também faz fallback paginado no histórico, filtrando pela consulta exata e pela janela atual.
    """
    inicio_busca_ts = time.time()
    try:
        request_id = create_request(item, valor)
    except Exception as e:
        return item, {"success": False, "blocked": True, "error": str(e)}, []
    ...
    if not cnjs and not (isinstance(resultado, dict) and resultado.get("pending_timeout")):
        lista = listar_requests_fallback(item, valor=valor, inicio_busca_ts=inicio_busca_ts)
...
def buscar_cnjs(valor, tipo, nacional=False):
    ...
    set_current_search_context(uf=uf, nacional=nacional)
    consultas = find_queries(param_keys, uf=uf, nacional=nacional)

    # Trava final: /oab regional NUNCA pode escapar para TJs de outros estados.
    # Ex.: /oab 110125--RS não pode criar request em tjsp/tjms/tjrj/tjdft/tjes.
    if tipo == "oab" and not nacional and uf:
        permitidas = set(searches_regionais_por_uf(uf))
        consultas = [c for c in consultas if str(c.get("search", "")).lower().strip() in permitidas]
        print("\n=========== TRAVA FINAL /OAB REGIONAL ===========")
        print("BUILD:", globals().get("CONSULTABOT_BUILD", "BUILD_FIX_SEM_NOME"))
        print("UF:", uf)
        print("Permitidas:", ", ".join(sorted(permitidas)))
        print("Total final:", len(consultas))
        for c in consultas:
            print(f"{c.get('search')}/{c.get('query')}/{c.get('param_key')} | platform={c.get('platform')}")
        print("=========== FIM TRAVA FINAL /OAB REGIONAL ===========\n")
