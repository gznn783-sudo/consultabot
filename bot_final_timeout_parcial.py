...
def ordenar_por_uf(consultas, uf=None):
    ...
    O erro que impedia TJRS/TJSC era aqui: a versão anterior retornava
    apenas `filtradas`, ou seja, cortava todos os outros tribunais.
    Agora o fluxo segue a documentação: usa /available inteiro, só reordena.
    """
    if not uf:
        return consultas
...
def processar_rota_busca(item, valor):
    ...
    Usado em paralelo para a busca OAB não ficar presa em TJRS/TJSC/TRF4 pending.
    """
    ...
    # Fallback só se a rota finalizou sem CNJ.
    # Se está pending_timeout, não adianta listar histórico imediatamente.
