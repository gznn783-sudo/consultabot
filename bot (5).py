...
def ordenar_por_uf(consultas, uf=None):
    """
    Prioriza tribunais da UF, mas NÃO remove os demais.

    Antes o código filtrava só TJRS/TRF4, TJSC/TRF4 etc. Isso fazia a busca
    perder resultados quando a Codilo tinha outro search/plataforma habilitada
    para OAB. Agora ele coloca os mais prováveis primeiro e mantém todo o resto.
    """
    if not uf:
        return consultas
    ...
    def chave_ordem(item):
        ...
        if search in prioridade:
            return (0, prioridade.index(search))
        return (1, 999)

    return sorted(consultas, key=chave_ordem)
...
def extrair_requests_do_autorequest(data):
    """Extrai a lista de requests internos do retorno do /v1/autorequest/{id}."""
...
def consultar_autorequest(auto_id, max_tentativas=8, tempo_espera=8):
    """
    Consulta o AutoRequest por tempo limitado.

    Não fica esperando 1 hora: por padrão são 8 tentativas x 8s.
    Retorna requests success e também a lista inteira para mostrar status interno.
    """
    ...
    for tentativa in range(max_tentativas):
        ...
        requests_list = extrair_requests_do_autorequest(data)
        ...
        if success_requests:
            return success_requests, requests_list, ultimo

        # Se todos já terminaram sem sucesso, não adianta esperar mais.
        if requests_list:
            statuses = [str(r.get("status", "")).lower() for r in requests_list]
            if statuses and all(s in ["warning", "error", "failed", "failure"] for s in statuses):
                return [], requests_list, ultimo
...
def resumo_status_requests(requests_list):
    if not requests_list:
        return "Sem requests internos retornados."

    linhas = []
    for r in requests_list[:12]:
        tribunal = r.get("court") or r.get("search") or r.get("tribunal") or "tribunal"
        status = r.get("status") or "sem status"
        linhas.append(f"{tribunal}: {status}")

    if len(requests_list) > 12:
        linhas.append(f"... +{len(requests_list) - 12} requests")

    return "\n".join(linhas)


def extrair_primeira_capa_de_raw(raw, cnj, tribunal="Não informado"):
    processos = extrair_lista_processos(raw)
    log_json("PROCESSOS EXTRAIDOS PARA CAPA", processos)
...
def buscar_detalhes_rapido(cnj, tribunal=None):
    ...
    # 1) Primeiro tenta consulta direta no tribunal do botão.
    ...
    # 2) Depois usa Consulta Automática por CNJ, conforme documentação Codilo.
    try:
        auto_id = criar_autorequest(cnj)
        success_requests, all_requests, raw_auto = consultar_autorequest(
            auto_id,
            max_tentativas=8,
            tempo_espera=8,
        )

        status_interno = resumo_status_requests(all_requests)

        # 2.1) Às vezes o próprio autorequest já traz data/capa.
        capa_auto = extrair_primeira_capa_de_raw(raw_auto, cnj, tribunal or "Não informado")
        if capa_auto:
            DETAIL_CACHE[cache_key] = capa_auto
            return capa_auto

        # 2.2) Se houver requests internas success, abre cada /request/{id}.
        for req in success_requests:
            ...
            resultado = get_request_result(req_id, max_tentativas=4, tempo_espera=6)
            raw = resultado.get("raw", resultado) if isinstance(resultado, dict) else resultado

            capa = extrair_primeira_capa_de_raw(raw, cnj, tribunal_req)
            if capa:
                DETAIL_CACHE[cache_key] = capa
                return capa

    except Exception as e:
        status_interno = f"Erro no AutoRequest: {str(e)[:500]}"
        print("ERRO buscar_detalhes_rapido/autorequest:", str(e))

    resposta_minima = (
        formatar_capa_minima(cnj, tribunal or "Não informado")
        + "\n\n"
        + f"AutoRequest ID: {auto_id or 'Não gerado'}\n\n"
        + "Status interno:\n"
        + status_interno
        + "\n\n"
        + "Clique em 🔁 Tentar novamente em alguns minutos para consultar o mesmo ID outra vez."
    )

    # Não guarda capa mínima no cache para permitir tentar novamente depois.
...
async def callback_detalhe(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        teclado = None
        if "⚠️" in resposta or "Ainda" in resposta or "Status interno" in resposta:
            teclado = InlineKeyboardMarkup([
                [InlineKeyboardButton("🔁 Tentar novamente", callback_data=f"detalhe:{cnj}:{tribunal}")]
            ])
