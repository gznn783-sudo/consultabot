...
MAX_CODILO_REQUESTS = int(os.getenv("MAX_CODILO_REQUESTS", "0"))
CODILO_MAX_POLL_ATTEMPTS = int(os.getenv("CODILO_MAX_POLL_ATTEMPTS", "35"))
CODILO_POLL_SLEEP = float(os.getenv("CODILO_POLL_SLEEP", "3"))
CODILO_CONCURRENCY = int(os.getenv("CODILO_CONCURRENCY", "4"))
...
def find_queries(param_keys, uf=None):
    ...
    # IMPORTANTE: não filtra por UF.
    # A UF da OAB serve só para priorizar (ex.: RS primeiro), mas a busca precisa
    # consultar TODAS as rotas que o /available informar com tag=oab.
    # Antes o código cortava para UF_TRIBUNAIS e podia deixar de fora TRT, PJe,
    # Projudi, e outras rotas que também retornam processos do advogado.
...
def executar_consulta_codilo(item, valor):
    """
    Cria uma requisição individual na Codilo e faz polling até sair de pending
    ou atingir CODILO_MAX_POLL_ATTEMPTS. Retorna sempre um pacote seguro,
    sem derrubar a busca inteira quando um tribunal trava.
    """
    tribunal_item = item.get("search", "Não informado")
    query_item = item.get("query", "Não informado")
    chave_stat = f"{tribunal_item}/{query_item}"
...
def buscar_cnjs(valor, tipo):
    ...
    print(f"TOTAL DE ROTAS SELECIONADAS: {len(consultas)}")
    print(f"CODILO_MAX_POLL_ATTEMPTS={CODILO_MAX_POLL_ATTEMPTS} | CODILO_POLL_SLEEP={CODILO_POLL_SLEEP} | CODILO_CONCURRENCY={CODILO_CONCURRENCY}")
    for c in consultas:
        print(f"{c.get('search')}/{c.get('query')} param={c.get('param_key')}")
    print("=========== FIM CONSULTAS SELECIONADAS ===========\n")

    # Ponto principal da correção:
    # antes o bot criava uma consulta e ficava preso nela até terminar as tentativas.
    # Quando TJRS/TRF/TRT ficava pending, a busca inteira demorava demais e parecia
    # retornar sempre só os mesmos 165. Agora as consultas rodam em lotes paralelos:
    # tribunal travado não bloqueia os outros tribunais.
    workers = max(1, CODILO_CONCURRENCY)

    with ThreadPoolExecutor(max_workers=workers) as executor:
        futuros = [executor.submit(executar_consulta_codilo, item, valor) for item in consultas]

        for futuro in as_completed(futuros):
            pacote = futuro.result()
            item = pacote.get("item") or {}
            tribunal_item = pacote.get("tribunal") or item.get("search", "Não informado")
            chave_stat = pacote.get("chave_stat") or f"{tribunal_item}/{item.get('query')}"
            cnjs = pacote.get("cnjs") or []
            status = pacote.get("status") or ""

            if pacote.get("erro"):
                erros.append(pacote["erro"])
                continue

            if not cnjs:
                if status in ["pending", "processing", "running", "waiting", "created"]:
                    erros.append(f"{chave_stat}: pending/timeout sem CNJ após {CODILO_MAX_POLL_ATTEMPTS} tentativas")
                elif status in ["warning", "error", "failed", "failure"]:
                    erros.append(f"{chave_stat}: {status} sem CNJ")
                continue
